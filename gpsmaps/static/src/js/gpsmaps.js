...
"use strict";
    ...
    var HomePage = instance.Widget.extend({
        ...
        start: function () {
            ...
            if (odoo.gpsmaps_var.refposiciones.length > 1) odoo.gpsmaps_var.refposiciones.push(timer_position);
            else odoo.gpsmaps_var.refposiciones[1] = timer_position;
        ...
        process: function (ObjetoOdoo, map) {
            ...
            for (ivehiculo in odoo.gpsmaps_var.posicion) {
                ...
                if (data_vehiculo != undefined) {
                    ...
                    if (odoo.gpsmaps_var.locations[ivehiculo] == undefined) odoo.gpsmaps_var.locations[ivehiculo] = Array(location);
                    else odoo.gpsmaps_var.locations[ivehiculo].push(location);
                }
            }
        }
    });
    ...
    local.history = instance.Widget.extend({
        ...
        paint_history: function (ObjetoOdoo, map, search, position, time) {
            ...
            if (time == 0) {
                for (ivehiculo in odoo.gpsmaps_var.posiciones) {
                ...
                if (odoo.gpsmaps_var.posiciones.length > 0 && odoo.gpsmaps_var.detener == 0) {
                    ...
                    if (data_vehiculo != undefined) {
                        ...
                        if (odoo.gpsmaps_var.locations[position] == undefined) odoo.gpsmaps_var.locations[position] = Array(location);
                        else odoo.gpsmaps_var.locations[position].push(location);
                        if (odoo.gpsmaps_var.posiciones.length - 1 > position) {
                            ...
                            if (vehiculo["speed"] == 0) {
                                ...
                                if (odoo.gpsmaps_var.otras_locations == undefined) odoo.gpsmaps_var.otras_locations = Array(location);
                                else odoo.gpsmaps_var.otras_locations.push(location);
                            }
                            setTimeout(function () {
                                position = position + 1;
            ...
            var obj_positions = new instance.web.Model('gpsmaps.positions')
                ...
                .then(function (results) {
                    _(results).each(function (item) {
                        ...
                        map = ObjetoOdoo.CreateMap(16, "ROADMAP", {
    ...
    local.geofence = instance.Widget.extend({
        start: function () {
            ...
            google.maps.event.addListener(map, 'click', function (event) {
                ...
                if (odoo.gpsmaps_var.geofence.length > 0) ObjetoOdoo.poliline(map, odoo.gpsmaps_var.geofences);
        ...
        points: function () {
            var points;
            for (ipoint in odoo.gpsmaps_var.geofence) {
    ...
    local.gpsmaps = instance.Widget.extend({
        ...
        codeLatLng: function (coordinates) {
            geocoder = odoo.gpsmaps_var.geocoder;
        ...
        del_locations_otras: function () {
            if (odoo.gpsmaps_var.otras_locations.length > 1) {
                ...
                for (idvehicle in odoo.gpsmaps_var.otras_locations) {
        ...
        clearTimer: function () {
            for (ipos in odoo.gpsmaps_var.refposiciones) {
        ...
        positions_history: function (map, search) {
            if ($('#map').length) {
                ...
                var obj_positions = new instance.web
                    ...
                    .then(function (results) {
                        _(results).each(function (item) {
                            if (odoo.gpsmaps_var.posiciones.length > 0) odoo.gpsmaps_var.posiciones.push(item);
                            else odoo.gpsmaps_var.posiciones[1] = item;
                        });
                    });
            }
            else this.clearTimer();
